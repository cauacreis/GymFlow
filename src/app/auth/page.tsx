    ...
    try {
      ...
      if (client) {
        ...
        if (redirectTarget && redirectTarget !== "/") {
          searchParams.set("next", redirectTarget);
        }
        const queryString = searchParams.toString();
        const callbackPath = queryString ? `/auth/callback?${queryString}` : "/auth/callback";
      ...
      if (err instanceof Error) {
        ...
        setErrorMessage(`Erro ao autenticar com ${provider === "google" ? "Google" : "Apple"}.`);
      }
    } finally {
      setIsLoading(false);
    }
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full h-10 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.1] hover:border-white/[0.18] text-white font-semibold text-xs flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-50"
                >
