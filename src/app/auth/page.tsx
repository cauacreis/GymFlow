...
function AuthPageContent() {
  ...
  // E-mail lembrado e captura de erros/códigos de retorno OAuth
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (authErr) {
        setErrorMessage(
          authErr.includes("not enabled")
            ? "O provedor social precisa ser ativado no painel do Supabase com Client ID e Secret."
            : translateSupabaseError(authErr)
        );
      ...
      if (client) {
        ...
        if (error) {
          ...
          if (lower.includes("redirect_uri") || lower.includes("redirect_to") || lower.includes("not allowed")) {
            throw new Error(`URL de redirecionamento não autorizada no Supabase. Adicione '${window.location.origin}/auth/callback' nas configurações de URL.`);
          }
          throw new Error(translateSupabaseError(error.message));
        }
      } else {
        triggerHaptic("warning");
        setErrorMessage("Autenticação social requer chaves ativas do Supabase no ambiente.");
      }
    } catch (err: unknown) {
      triggerHaptic("warning");
